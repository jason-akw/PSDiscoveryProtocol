...
echo } >> "%TEMP_PS1%"
echo function Convert-DisplayValue { >> "%TEMP_PS1%"
echo     param([object]$Value) >> "%TEMP_PS1%"
echo     if ($null -eq $Value) { return '' } >> "%TEMP_PS1%"
echo     if ($Value -is [string]) { return $Value } >> "%TEMP_PS1%"
echo     if ($Value -is [System.Collections.IEnumerable] -and -not ($Value -is [string])) { >> "%TEMP_PS1%"
echo         $parts = @() >> "%TEMP_PS1%"
echo         foreach ($entry in $Value) { >> "%TEMP_PS1%"
echo             if ($entry -is [string]) { $parts += $entry } >> "%TEMP_PS1%"
echo             elseif ($entry -is [ValueType]) { $parts += [string]$entry } >> "%TEMP_PS1%"
echo             else { $parts += ($entry ^| ConvertTo-Json -Compress -Depth 6) } >> "%TEMP_PS1%"
echo         } >> "%TEMP_PS1%"
echo         return ($parts -join ', ') >> "%TEMP_PS1%"
echo     } >> "%TEMP_PS1%"
echo     if ($Value -is [ValueType]) { return [string]$Value } >> "%TEMP_PS1%"
echo     return ($Value ^| ConvertTo-Json -Compress -Depth 6) >> "%TEMP_PS1%"
echo } >> "%TEMP_PS1%"
echo function Show-DiscoveryResult { >> "%TEMP_PS1%"
echo     param([psobject]$Item, [string[]]$Fields) >> "%TEMP_PS1%"
echo     foreach ($name in $Fields) { >> "%TEMP_PS1%"
echo         $raw = $Item.PSObject.Properties[$name].Value >> "%TEMP_PS1%"
echo         $text = Convert-DisplayValue -Value $raw >> "%TEMP_PS1%"
echo         Write-Host ($name + ': ' + $text) >> "%TEMP_PS1%"
echo     } >> "%TEMP_PS1%"
echo } >> "%TEMP_PS1%"
echo do { >> "%TEMP_PS1%"
...
echo                 $fields = 'Device','SystemName','SoftwareVersion','Model','IPAddress','Management','VLAN','Port','Capabilities','Duplex','TrustBitmap','UntrustedPortCoS','Connection','Interface','Computer','Type' >> "%TEMP_PS1%"
echo                 Show-DiscoveryResult -Item $item -Fields $fields >> "%TEMP_PS1%"
echo             } else { >> "%TEMP_PS1%"
echo                 $fields = 'Device','SystemName','SystemDescription','Model','IPAddress','ManagementAddresses','VLAN','VLANNamedEntries','Port','PortDescription','ChassisId','ChassisIdSubtype','ChassisIdSubtypeName','PortIdSubtype','PortIdSubtypeName','TimeToLive','SystemCapabilities','EnabledCapabilities','LinkAggregation','MacPhyConfigurationStatus','ManagementInterfaceNumberingSubtype','ManagementInterfaceNumber','ManagementObjectIdentifier','Connection','Interface','Computer','Type' >> "%TEMP_PS1%"
echo                 Show-DiscoveryResult -Item $item -Fields $fields >> "%TEMP_PS1%"
echo             } >> "%TEMP_PS1%"
echo             } >> "%TEMP_PS1%"
echo         } else { >> "%TEMP_PS1%"
